package cn.iocoder.basic.module.infra.service.codegen.inner;
...
import cn.iocoder.basic.framework.apilog.core.enums.OperateTypeEnum;
...
import cn.iocoder.basic.framework.excel.core.annotations.DictFormat;
import cn.iocoder.basic.framework.excel.core.convert.DictConvert;
...
import cn.iocoder.basic.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import cn.iocoder.basic.module.infra.dal.dataobject.codegen.CodegenTableDO;
import cn.iocoder.basic.module.infra.enums.codegen.CodegenFrontTypeEnum;
import cn.iocoder.basic.module.infra.enums.codegen.CodegenSceneEnum;
import cn.iocoder.basic.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import cn.iocoder.basic.module.infra.enums.codegen.CodegenVOTypeEnum;
import cn.iocoder.basic.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 basic-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 basic-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.iocoder.basic.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/basic-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/basic-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "basic-module-${table.moduleName}/" + // 顶级模块
                "basic-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "basic-module-${table.moduleName}/" + // 顶级模块
                "basic-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "basic-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "basic-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "basic-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
